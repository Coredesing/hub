...
build-frontend-user:
	make cd-server-sotatek-starter && cd ./frontend-user && cp .env.sotatek.example .env && yarn && npx patch-package && yarn build && pm2 restart SotatekStarter
...
create-pm2-frontend-user:
	make cd-server-sotatek-starter && cd ./frontend-user && pm2 start "serve -s build -l 8333" --name=SotatekStarter

create-pm2-frontend-admin:
	make cd-server-sotatek-starter && cd ./frontend-admin && pm2 start "serve -s build -l 8667" --name=SotatekStarterFrontEndAdmin
