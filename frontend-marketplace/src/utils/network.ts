import { ETH_CHAIN_ID, BSC_CHAIN_ID, POLYGON_CHAIN_ID, APP_NETWORKS_SUPPORT, NetworkInfo, ChainDefault } from '../constants/network';
...
export const getExplorerTransactionLink = ({ appChainID = ChainDefault.id, transactionHash }: { appChainID?: string, transactionHash?: string }) => {
  const info = getNetworkInfo(appChainID as string);
...
export const getNetworkInfo = (network: string | number): NetworkInfo => {
  let appChainID = ChainDefault.id as string;
