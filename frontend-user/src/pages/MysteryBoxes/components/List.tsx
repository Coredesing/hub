...
const List = () => {
  ...
  const url = useMemo(() => {
    return `/pools/mysterious-box?token_type=${TOKEN_TYPE.Box}&campaign_status=Ended&network_available=${network}&title=${title}&is_display=1&page=${page}&limit=6&is_search=1`
  }, [network, title, page])
  ...
  return (
    <div className={styles.container} style={{marginBottom: '2rem'}}>
      ...
      <div className={styles.pagination} style={{marginTop: '2rem'}}>
        ...
        <span onClick={(e) => { page > 1 && setPage(page - 1) }}>
        ...
        { pages && (
          <>
          <span className={pages.current === pages.first ? styles.paginationActive : ''} onClick={(e) => { setPage(pages.first) }}>{pages.first}</span>
          { pages.leftCluster && <span>...</span> }
          { pages.pages.map(page => <span key={page} className={pages.current === page ? styles.paginationActive : ''} onClick={(e) => { setPage(page) }}>{page}</span>) }
          { pages.rightCluster && <span>...</span> }
          { pages.last > pages.first && <span className={pages.current === pages.last ? styles.paginationActive : ''} onClick={(e) => { setPage(pageLast) }}>{pages.last}</span> }
          </>
        )}
        <span onClick={(e) => { page < pageLast && setPage(page + 1) }}>
          <svg width="6" height="8" viewBox="0 0 6 8" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M1.25 0.25L5 4L1.25 7.75" stroke="white" strokeMiterlimit="10" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </span>
        <span onClick={(e) => { setPage(pageLast) }}>
