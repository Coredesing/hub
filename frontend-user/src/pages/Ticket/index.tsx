...
const Ticket: React.FC<any> = (props: any) => {
  ...
  const {
    ...
    transactionHash,
    claimTokenSuccess,
  } = useTokenClaim(infoTicket.campaign_hash, infoTicket?.id);
  ...
  return (
    <DefaultLayout>
      <TicketModal open={openModal} onClose={onCloseModal} transaction={tokenDepositTransaction || transactionHash} networkName={infoTicket?.network_available} />
          ...
          <div className={styles.cardBody}>
            <div className={styles.cardBodyText}>
              ...
              <button className={clsx({ openBuy: isShowInfo })}>
                ...
                <span>{isClaim ? 0 : infoTicket.ether_conversion_rate} {infoTicket && infoTicket.accept_currency ? infoTicket.accept_currency.toUpperCase() : '...'}</span>
            ...
            <div className={styles.cardBodyDetail}>
              ...
              {endOpenTime && !infoTicket.campaign_hash && <div className={styles.comingSoon}>Coming soon</div>}
              {infoTicket.campaign_hash && isShowInfo && <div className={styles.cardBodyProgress}>
                ...
                {
                  !finishedTime && <div className={styles.infoTicket}>
                ...
                {
                  finishedTime && <div className={styles.infoTicket}>
                    <span className={styles.text}>TOTAL SALES</span> <span className={styles.textBold}>
                ...
                {
                  !finishedTime &&
                    (isClaim ? <button className={clsx(styles.buynow, {
                      [styles.buyDisabled]: numTicketBuy <= 0
                    })} onClick={onClaimTicket}>
                    Claim
                  </button> : <>
                    {allowNetwork && isBuy && isAccApproved(tokenAllowance || 0) && <div className={clsx(styles.infoTicket, styles.buyBox)} style={{ marginTop: '16px' }}>
                      <div className={styles.amountBuy}>
                        <span>Amount</span>
                        <div>
                          <span onClick={descMinAmount} className={clsx(styles.btnMinMax, 'min', {
                            disabled: !getMaxTicketBuy(ticketBought, +infoTicket.max_buy_ticket) || numTicketBuy === 0
                          })}>Min</span>
                          <span onClick={descAmount} className={clsx({
                            [styles.disabledAct]: !getMaxTicketBuy(ticketBought, +infoTicket.max_buy_ticket) || numTicketBuy === 0
                              ...
                              <path d="M11.1818 0H0.818182C0.366544 0 0 0.366544 0 0.818182V1.3636C0 1.81524 0.366544 2.18178 0.818182 2.18178H11.1818C11.6334 2.18178 12 1.81524 12 1.3636V0.818182C12 0.366544 11.6334 0 11.1818 0Z" fill="white" />
                            </svg>
                          </span>
                          <span>
                            {numTicketBuy}
                          ...
                          <span onClick={ascMaxAmount} className={clsx(styles.btnMinMax, 'max', {
                            disabled: !getMaxTicketBuy(ticketBought, +infoTicket.max_buy_ticket) || numTicketBuy === getMaxTicketBuy(ticketBought, +infoTicket.max_buy_ticket)
                          })}>Max</span>
                        </div>
                      </div>
                      <button className={clsx(styles.buynow, {
