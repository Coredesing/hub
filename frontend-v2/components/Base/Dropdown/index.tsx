...
const Dropdown = ({ items, selected, onChange, propLabel, propValue, isFilter, children, classes, propIcon, isShow, onHandleFilter, disabled }: Props) => {
  ...
  return (
    <div className={`relative inline-block text-sm border border-transparent ${classes?.wrapperDropdown || ''}`}>
      ...
      {
        !isFilter
          ? <button className={`${styles.button} ${classes?.buttonSelected || ''} flex align-middle justify-between items-center bg-gamefiDark-630/30 ${disabled ? 'text-white/50' : 'text-white'} font-bold uppercase px-4 rounded mr-2 w-full ${disabled ? 'cursor-not-allowed' : ''}`} onClick={() => !disabled && setShow(!show)}>
