...
const Dropdown = ({ items, selected, onChange, propLabel, propValue, isFilter, children }: Props) => {
  ...
  return (
    <div className="relative inline-block text-sm border border-transparent">
      <div className={`absolute left-0 mt-2 rounded-r ${show ? 'bg-gamefiGreen-700' : 'bg-gamefiDark-300'}`} style={{ width: '2px', height: '20px' }}></div>
      {
        !isFilter
          ? <button className="flex align-middle items-center bg-gamefiDark-650 text-white font-bold uppercase px-4 py-2 rounded mr-2" onClick={() => setShow(!show)}>
            ...
            : <div className="w-full">
              {
                availableOptions() && availableOptions().length
                  ? availableOptions().map(item =>
                    <button key={item[propValue] || item.value} onClick={() => handleChangeFilter(item)} className="cursor-pointer hover:bg-gamefiDark-600 px-4 py-1 w-full text-left text-sm">{item[propLabel] || item.label}</button>
                  )
                  : <></>
              }
            </div>
