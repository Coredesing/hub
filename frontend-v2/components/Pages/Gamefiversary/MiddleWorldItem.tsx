  ...
  return (
    <div className="flex flex-col h-[600px] md:h-auto md:flex-1 bg-[#1B1D26] relative">
      <div className="w-full p-6 pt-8 flex items-center justify-between">
        <span className='font-mechanic font-bold text-[20px] leading-[100%] uppercase text-white'>{data.name}</span>
        { data?.tutorialUrl && <img src={present.src} alt="present" className=''/>}
      </div>
      <div className="flex-1 flex flex-col overflow-y-scroll md:mr-2 gap-2 pb-30 md:pb-0">
        {data?.tasks?.length > 0 &&
          data?.tasks.filter(e => !e.name.includes('dummy')).map((task, iTask) => (
