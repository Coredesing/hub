          ...
          {posts.map(post => <div className="mb-8 w-full" key={`twitter-${post.id}`}>
            <div>
              <div key={post.id} className="w-full bg-gamefiDark-800 rounded px-6 py-4 font-casual leading-6 h-full">
                ...
                {!!post?.created_at && <a href={`https://twitter.com/${user?.username}/status/${post?.id}`} target="_blank" rel="noreferrer" className="hover:underline text-sm font-medium text-gamefiDark-200 cursor-pointer">{format(new Date(post?.created_at), 'dd MMM yyyy')}</a>}
              </div>
            </div>
          </div>)}
          <ViewportSlot>
