...
const GameCarousel = ({ items }: Props) => {
  ...
  useEffect(() => {
    ...
    setMobileSliderPlugins([new Sync({
      type: 'index',
      synchronizedFlickingOptions: [
        {
  ...
  useEffect(() => {
    if (!currentItem?.poolInfo?.saleStartTime && ['tba', 'upcoming'].includes(currentItem?.poolInfo?.campaignStatus?.toLowerCase())) {
      return setCountDown({ title: 'Whitelist Starts In', toTime: 'TBA' })
    }
    if (currentItem?.poolInfo?.saleEndTime && currentItem?.poolInfo?.campaignStatus?.toLowerCase() === 'swap') {
      return setCountDown({ title: 'Swap Ends In', toTime: currentItem?.poolInfo?.saleEndTime })
    }
    if (currentItem?.poolInfo?.buyType?.toLowerCase() === 'whitelist' &&
    currentItem?.poolInfo?.campaignStatus?.toLowerCase() === 'upcoming' &&
    now?.getTime() >= new Date(Number(currentItem?.poolInfo?.whitelistStartTime) * 1000).getTime() &&
    now?.getTime() < new Date(Number(currentItem?.poolInfo?.whitelistEndTime) * 1000).getTime()) {
      return setCountDown({ title: 'Whitelist Ends In', toTime: currentItem?.poolInfo?.whitelistEndTime })
    }
    if (currentItem?.poolInfo?.buyType?.toLowerCase() === 'whitelist' &&
    now?.getTime() < new Date(Number(currentItem?.poolInfo?.whitelistStartTime) * 1000).getTime()) {
      return setCountDown({ title: 'Whitelist Starts In', toTime: currentItem?.poolInfo?.whitelistStartTime })
    }
    if (now?.getTime() > new Date(Number(currentItem?.poolInfo?.whitelistEndTime) * 1000).getTime() &&
    now?.getTime() <= new Date(Number(currentItem?.poolInfo?.saleStartTime) * 1000).getTime()) {
      return setCountDown({ title: 'Buying Phase Starts In', toTime: currentItem?.poolInfo?.saleStartTime })
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentItem])
    ...
    <>
              ...
              <div id='flicking2wrapper' className={clsx(!isCurVideoPlaying ? 'right-6 opacity-100' : '-right-24 opacity-0', 'absolute top-6 w-32 z-20 group-hover:right-6 group-hover:opacity-100 transition-all duration-300 ease-in-out')} style={{ height: `${flickingVerticalHeight}px` }}>
                <Flicking onChanged={e => setCurPanelIndex(e.index)} className="h-full" horizontal={false} ref={refSlider1} defaultIndex={curPanelIndex} bound={true} bounce={30} moveType="freeScroll">
                  {items.map((item, i) => <div key={`caurouse-paginate-${item.id}`} className={clsx('my-1 cursor-pointer rounded-sm overflow-hidden relative border border-transparent hover:border-gamefiGreen-700', curPanelIndex === i ? 'border border-gamefiGreen-700' : '')}>
                    <img className="h-20 w-36 object-cover thumbnail-bullet-active" src={item?.thumbnail} alt=""></img>
              ...
              <div className="absolute md:bottom-8 lg:bottom-14 2xl:bottom-32 left-8 z-30">
                <div className="flex align-middle items-center w-full mb-5">
                  { <div className="flex align-middle items-center text-sm">
                    ...
                    <p className="ml-[10px] tracking-widest text-base text-white leading-[150%]">{currentItem?.likes || 0}</p>
                  </div> }
                  {
                    // get(currentItem, 'develop') && <div className="flex align-middle items-center ml-[30px] text-left">
                    ...
                    //   >{get(currentItem, 'develop')}</p>
                    // </div>
                  }
                </div>
                <div className="text-[40px] font-bold uppercase text-left mb-8  leading-[100%]">{currentItem?.title}</div>
                { currentItem.upcoming && <div className='uppercase mt-8 font-bold text-[13px] tracking-[0.04em] leading-[150%] mb-1'>{countdown?.title || ''}</div> }
                {
                  currentItem.upcoming && countdown?.toTime !== 'TBA' && <Countdown to={countdown.toTime}/>
                }
                {
                  currentItem.upcoming && countdown?.toTime === 'TBA' && <div className='font-casual pb-3'>TBA</div>
                }
                <div>
                  <Link href={currentItem.upcoming ? `/igo/${currentItem?.slug}` : `/hub/${currentItem?.aggregatorSlug}`} passHref>
                    <div className="bg-gamefiGreen-700 text-gamefiDark-900 py-2 px-6 rounded-xs clipped-t-r hover:opacity-90 w-36 cursor-pointer">
                      <a className="flex align-middle items-center">
                        <div className="mr-2 uppercase font-bold text-[13px]">{currentItem.upcoming ? 'Join now' : 'View more'}</div>
      ...
      <div className="lg:hidden">
        ...
        <div className="flex w-full h-full items-center gap-4">
          <Flicking circular={true} plugins={mobileSliderPlugins} className="w-full h-full flex" align="center" ref={refSliderMobile} interruptable={true}>
            {items.map(item => (
              <div key={`mobile-game-${item.id}`} className="w-full">
                <div className="w-full">
                  <div className="absolute z-20 top-0 left-0 uppercase font-medium tracking-widest md:text-xs xl:text-sm text-center md:text-left bg-gamefiDark-900 w-1/2 md:pb-1 lg:pb-2 clipped-b-r-full inline-block"><span className="text-gamefiGreen-700">Featured</span> games</div>
                  <video
                    id={`video-${item.id}`}
                    key={`video-${item.id}`}
                ...
                <div className="w-full my-4 px-8">
                  <div className="text-lg xl:text-xl 2xl:text-3xl font-bold uppercase text-left">{item.title}</div>
                  <div className="flex align-middle items-center w-full mt-3 xl:mt-5">
                    <div className="flex align-middle items-center text-sm">
                      <Image src={require('@/assets/images/icons/heart.svg')} alt="heart"/>
                      <p className="ml-2 tracking-widest text-gray-200">{currentItem?.likes || 0}</p>
                    </div>
                  ...
                  <div className="mt-3">
                    <p className="font-casual text-left leading-5 md:text-xs lg:text-base text-gray-300 max-h-36 overflow-y-scroll">{item.shortDescription}</p>
                  </div>
                  <div className="mt-3">
        ...
        <Flicking ref={refSliderMobile1} bound={true} bounce={30} moveType="freeScroll">
