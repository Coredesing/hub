...
import { useHubDetailContext } from '../utils'
...
const TwitterFeed = () => {
  ...
  const fetchNews = useCallback(async () => {
    try {
      const response = await fetcher(`/api/hub/detail/feeds?id=${hubData?.project?.data?.attributes?.twitterId}&limit=${PER_PAGE}`)
      setPosts(response?.data?.posts?.data)
      setUser(response?.data?.user?.data)
    } catch (e) {}
  }, [hubData?.project?.data?.attributes?.twitterId])
  ...
  return (
    posts?.length > 0 && <> <div className="mt-10 md:mt-14 text-lg md:text-2xl font-mechanic uppercase"><strong>Twitter Feed</strong></div>
      <div className="flex w-full h-full xl:grid xl:grid-cols-3 overflow-x-auto hide-scrollbar gap-5">
        {posts.slice(0, 3).map(post => <div key={post.id} className="flex flex-col bg-gamefiDark-800 rounded px-6 py-4 font-casual leading-6 min-w-full">
          ...
          <div className="w-full text-sm mb-4">
            {!!post.text && <TwitterParser>{post.text}</TwitterParser>}
          </div>
          {!!post?.created_at && <div className="text-sm font-medium text-gamefiDark-200 mt-auto">{format(new Date(post?.created_at), 'dd MMM yyyy')}</div>}
        </div>)}
      </div>
    </>
  )
