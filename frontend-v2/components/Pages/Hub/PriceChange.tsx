const PriceChange = ({ priceChange24h, className }: { priceChange24h: string; className: string }) => {
  if (!priceChange24h) {
    return null
  }
  const priceChange = parseFloat(priceChange24h)

  return <span className={`${className} inline-flex items-center px-2 rounded ${priceChange >= 0 ? 'bg-gamefiGreen-500 text-gamefiDark-800' : 'bg-red-500'}`}>
    {priceChange >= 0
