...
export class ScrollStage {
  constructor (el, elCanvas, onScroll) {
    this.scr = onScroll
    this.element = el
   ...
   */
  addCanvas () {
    this.canvas.classList.add(styles.webgl)
  ...
  updateScrollAnimations () {
    ...
    if (this.scr) {
      this.scr(this.scroll.normalized)
    }
    ...
    for (const key in this.settings) {
      if (this.settings[key].start !== this.settings[key].end) {
        GSAP.to(this.mesh.material.uniforms[key], {
          value: Number(this.settings[key].start) + this.scroll.normalized * (this.settings[key].end - this.settings[key].start)
        })
      }
    }
  }
