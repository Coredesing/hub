  ...
  const approveToken = useCallback(async () => {
    ...
    try {
      if (token && spender && owner
        ...
        debugger
        const contract = getContract(token.address, ERC20_ABI, { accountSigner: account, network: { chainId: network.id, name: network.name } });
