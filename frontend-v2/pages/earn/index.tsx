...
const Earn = ({ pools: initPools }) => {
  ...
  useEffect(() => {
    const v = Object.keys(selected).reduce((acc, val) => {
      ...
      const remaining = cap.sub(staked)
      ...
      const remainingParsed = formatUnits(remaining, selected[val]?.decimals)
  ...
  const togglePool = useCallback((contractAddress) => {
    setModal(prevState => {
      return {
        ...prevState,
        [contractAddress]: !prevState[contractAddress]
        ...
        return <div key={contractAddress} className="rounded-sm shadow overflow-hidden">
          <div className="flex w-full items-center bg-gamefiDark-630/80 px-6 py-4">
            ...
            <div className="min-w-[12rem]">
              ...
              <div className="text-base uppercase font-medium font-casual my-0.5">{ printNumber(selectedExtended[contractAddress]?.remainingParsed) } GAFI</div>
            </div>
            <div>
              <p className="text-[13px] text-white font-bold uppercase text-opacity-50 mb-0.5">Lock-in term</p>
              { pools && <div className="flex text-[12px] gap-x-2 font-casual">{ pools.map(pool => <div key={pool.id} className={`px-2 py-1 rounded-sm border cursor-pointer ${selected[contractAddress] === pool ? 'border-gamefiGreen-500' : 'border-white/50'}`} onClick={() => {
                selectPool(contractAddress, pool)
              }}>
          ...
          { open[contractAddress] && selectedExtended[contractAddress] && <div className="bg-gamefiDark-630/30 p-4">
            <div className="flex px-2">
              ...
              <div className="min-w-[12rem] px-6 border-r border-white/20 flex flex-col">
                ...
                <div className="mt-auto">
                  { !networkIncorrect[contractAddress] && <>
                    <div onClick={() => togglePool(contractAddress)} className="bg-gamefiGreen-600 hover:bg-opacity-80 uppercase py-2 px-5 rounded-sm clipped-t-r text-[13px] font-bold text-center cursor-pointer text-gamefiDark-800">
                      Stake
                    </div>
                    <Modal
                      show={modal[contractAddress]}
                      toggle={() => togglePool(contractAddress)}
                    ...
                    >
                      <div className="px-8 pt-10 pb-6">
                        ...
                        <div className="flex justify-between mb-1">
                          <span className="text-[13px] text-white font-bold uppercase text-opacity-50">{selectedExtended[contractAddress].progress.toFixed(2)}%</span>
                          <span className="text-[13px] text-white font-bold uppercase">{printNumber(selectedExtended[contractAddress].capParsed)} {token}</span>
                        </div>
                        ...
                        <div className="flex gap-6 mt-4 pt-4 border-t border-white/20">
                          <div className="w-full">
                            ...
                            <p className="text-[13px] text-white font-bold uppercase text-opacity-50 mb-1">Amount ({token})</p>
                            <input
                              type="text"
                              ...
                              placeholder={`Minimum: ${printNumber(selectedExtended[contractAddress].amountMinParsed)} ${token}`}
                            />
                            <p className="text-xs font-casual leading-loose mb-6">Balance: 123.465 GAFI</p>
                          </div>
                          <div className="w-full">
                            ...
                            <div className="flex justify-between mb-6">
                              <div>
                                <p className="text-[13px] text-white font-bold uppercase text-opacity-50 mb-1">Estimated Interest ({token})</p>
                                <p className="font-casual text-sm">12.34 GAFI</p>
                              </div>
                              <div className="text-right">
                                <p className="text-[13px] text-white font-bold uppercase text-opacity-50 mb-1">APR</p>
                                <p className="font-casual text-sm">{ Number(selected[contractAddress]?.APR).toFixed(2) }%</p>
                              </div>
                            </div>
                            ...
                            <div className="flex justify-between mb-6">
                              ...
                              <div className="text-right">
                                ...
                                <p className="font-casual text-sm">{ format(addSeconds(new Date(), Number(selected[contractAddress].lockDuration)), 'yyyy-MM-dd') }</p>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="flex gap-6">
                          <div className="flex-1">
                            <div className="flex gap-2">
                              <div className="flex-1 bg-gamefiGreen-600 hover:bg-opacity-80 uppercase p-2 rounded-sm clipped-b-l text-[13px] font-bold text-center cursor-pointer text-gamefiDark-800">
                                Stake Now
                              </div>
                              <a
                                ...
                                href={selected[contractAddress]?.buyURL}
                                target="_blank"
                                rel="noreferrer"
                              >
                                <div className="bg-gamefiDark-650 clipped-t-r p-2 rounded-sm text-gamefiGreen-600 hover:text-opacity-80">Buy {token}</div>
                              </a>
                            </div>
                          </div>
