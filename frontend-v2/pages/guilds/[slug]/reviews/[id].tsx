...
function ReviewDetailPage ({ data }) {
  ...
  useEffect(() => {
    if (!data.id) router.replace('/guilds')
  ...
  return (
    <Layout title={'GameFi.org - Review'}>
      <div className="px-4 lg:px-24 md:container mx-auto lg:block">
        ...
        {data.id
          ? (
            <ReviewDetail data={data} currentResource="guilds" />
          )
          : (
...
export async function getServerSideProps ({ query }) {
  ...
  try {
    const result = await client.query({
      ...
      query: GET_REVIEW_BY_ID_FOR_GUILD
    })
    ...
    const gameName = get(reviewDetail, 'guild.data.attributes.name', '')
    const gameSlug = get(reviewDetail, 'guild.data.attributes.slug', '')
