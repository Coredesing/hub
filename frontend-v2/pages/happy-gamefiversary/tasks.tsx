...
>>>>>>> 6888d7c8c4bdf867c14f7326b7ccf38bf24e6fa9
  ...
  >
    ...
    <HubProvider><div ref={layoutBodyRef} className="w-full h-full">
      ...
      {/* GameFi world */}
      <section ref={gamefiWorldRef} className="w-full bg-[#111218] mt-9 pb-20 md:pb-0">
          ...
          <div className='max-w-[1180px] w-full pt-4 gap-1 mx-auto'>
            {gamefiTasks?.length > 0 && gamefiTasks.map((task, i) => <div key={`task-gamefi-${i}`} className='flex flex-col gap-2 w-full md:w-2/3 p-2 mx-auto'>
              <div className='flex flex-col md:flex-row w-full p-4 rounded gap-2 md:gap-10 bg-gradient-to-r from-[#292B36]/0 to-[#21232E]'>
                {(task?.socialInfo?.link || task?.slug === 'daily-checkin')
                  ? <a href={task?.slug === 'daily-checkin' ? 'https://gamefi.org/account/gxp' : task?.socialInfo?.link} target="_blank" rel="noreferrer" className='w-full md:w-1/3'>
                ...
                <div className='flex-1'>
                  <div className='flex items-center gap-4'>
                    ...
                    <div className="w-full max-w-[200px] h-2 bg-[#111218] rounded-sm overflow-hidden">
                      {!!(task?.currentRepetition / task?.stages?.[0]?.repetition) && <div className={`w-[${(task?.currentRepetition / task?.stages?.[0]?.repetition) * 100}%] h-full bg-white`}></div>}
                    </div>
