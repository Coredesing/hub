...
export async function getStaticProps () {
  const postsFeatured = await api.posts.browse({ include: 'authors,tags', limit: 4, filter: 'featured:true' }).catch(() => { })
  const postsLatest = await api.posts.browse({ limit: 4, filter: 'tag:-unlisted' }).catch(() => { })
  const postsUpdate = await api.posts.browse({ limit: 4, filter: 'tag:update' }).catch(() => { })
  const postsPartnership = await api.posts.browse({ limit: 4, filter: 'tag:partnership' }).catch(() => { })
  const postsIGO = await api.posts.browse({ limit: 6, filter: 'tag:igo' }).catch(() => { })
  const postsINO = await api.posts.browse({ limit: 6, filter: 'tag:ino' }).catch(() => { })
  const postsAMA = await api.posts.browse({ limit: 4, filter: 'tag:ama' }).catch(() => { })
  const tags = await api.tags.browse({ limit: 20, filter: 'visibility:public', include: 'count.posts', order: 'count.posts DESC' }).catch(() => { })

  return {
    props: {
      postsFeatured: cleanHTML(postsFeatured),
      postsLatest: cleanHTML(postsLatest),
      postsUpdate: cleanHTML(postsUpdate),
      postsPartnership: cleanHTML(postsPartnership),
      postsIGO: cleanHTML(postsIGO),
      postsINO: cleanHTML(postsINO),
      postsAMA: cleanHTML(postsAMA),
      tags
    },
    revalidate: 60
  }
