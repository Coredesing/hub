  ...
  return <div className="relative mb-14">
    ...
    <div className="flex flex-col sm:flex-row mt-6 gap-6">
      ...
      { items.others && <div className="w-full sm:w-1/2">
        { items.others.map(item => <div key={item.id} className="flex gap-4 mb-4">
          <Link href={`/news/${item.slug}`} passHref={true}>
          ...
          <div className="w-1/2">
            ...
            <Link href={`/news/${item.slug}`} passHref={true}>
              <a className="line-clamp-3 font-bold text-sm xl:text-lg !leading-none hover:underline">{item.title}</a>
            </Link>
          </div>
        </div>) }
      </div> }
  ...
  return <div className="relative mb-14">
    ...
    { items.others && <div className="grid grid-cols-2 mt-8 gap-6">
      { items.others.map(item => <div key={item.id} className="flex gap-4">
        ...
        <div className="w-1/2">
          <p className="font-casual text-[13px] text-white text-opacity-50 my-2">
            {format(new Date(item.published_at), 'MMM d, yyyy')}
            <span className="mx-2">•</span>
            {item.reading_time} min read
          </p>
          <Link href={`/news/${item.slug}`} passHref={true}>
            <a className="line-clamp-3 font-bold text-lg !leading-none hover:underline">{item.title}</a>
          </Link>
        </div>
      </div>) }
    </div>
