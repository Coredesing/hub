...
contract PreSaleNFTPool is Pausable, AccessControl {
    ...
    function claimTokens(address _candidate, uint256 _amount, bytes memory _signature) public whenNotPaused {
        ...
        token.mint(claimAmount, msg.sender);

        userClaimed[_candidate] += claimAmount;
        tokenClaimed += claimAmount;
